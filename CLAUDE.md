...
- A self-contained "line odds" tool for a friend. Off the chess curriculum, deliberately not linked from the NavBar (stays off-nav simply by not being in `NavBar.svelte`'s `SECTIONS`). Recreation of [Opening-Explorer-Plus](https://github.com/EikaMikiku/Opening-Explorer-Plus) by EikaMikiku — the page credits the original up front
- **What it does**: reports the cumulative probability that a whole opening line arises *from the starting position*, split by mover ("as White, you reach this position X% of the time given Black plays along"). This differs from a normal explorer's per-position frequency. Math ported from OEP: at each ply a move's conditional probability = its share of games there; multiply along the line, but split White's vs Black's moves (you control your own moves, only the opponent's replies are uncertain). A per-move "Skip" toggle treats a move as forced (probability 1, dropped from the product)
- **The one external network call in the app**: `src/lib/probabilitizer/lichess.ts` (`fetchExplorer`) hits the Lichess opening explorer API (`explorer.lichess.ovh`). Positions are encoded as a UCI move list via the `play=` param (from the standard start), so no FEN is needed for queries. Only chess positions are sent. Everywhere else the app has zero `fetch()` calls
- **Requires "Login with Lichess"**: since Feb 2026 (post-DDoS) the explorer rejects anonymous requests, so every request needs an OAuth Bearer token. `src/lib/probabilitizer/auth.ts` runs the Authorization Code + PKCE flow via `@bity/oauth2-auth-code-pkce` (Lichess's recommended lib) — fully client-side, no server, no client secret. We request **zero scopes** (the explorer only needs you authenticated, not account access). The user's token is stored in localStorage by the lib; `decorateFetchHTTPClient` wraps `fetch` to attach `Authorization: Bearer …`, passed into `fetchExplorer`. Endpoints: `lichess.org/oauth` + `lichess.org/api/token`. `clientId` is just a stable identifier (`howthehorseymoves`); `redirectUrl` is the current page minus query, so it works on localhost and prod without config. The page gates the whole tool behind a "Sign in with Lichess" button
- `probability.ts` — pure helpers: `moveProbability`, `movePlayRate`, `lineTotals` (products split by mover, honoring the Skip flag)
- `board-to-fen.ts` — `boardToFen()` serializer (the app's `parseFen` has no inverse); used only for the "open in Lichess analysis" link
- `src/routes/probabilitizer/+page.svelte` — reuses the shared `Board` component (`playableColors={['w','b']}`) for click/drag input, plus a paste-a-line box (parses SAN via `parseSan`, replays with a fetch per ply and a ~300ms gap to respect rate limits). Masters/Lichess DB toggle with rating + time-control filters. No persistence
