...
- **The one external network call in the app**: `src/lib/probabilitizer/lichess.ts` (`fetchExplorer`) hits the public Lichess opening explorer API (`explorer.lichess.ovh`, CORS `*`, same endpoint OEP uses). Positions are encoded as a UCI move list via the `play=` param (from the standard start), so no FEN is needed for queries. Only chess positions are sent — no student/app data. Everywhere else the app has zero `fetch()` calls
